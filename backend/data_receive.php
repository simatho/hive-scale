...
if(trim($_GET["weight"]) == "" || trim($_GET["temp_int"]) == "" || trim($_GET["chipmillis"]) == "" || trim($_GET["chipid"]) == "" || strlen(trim($_GET["mac"])) != 17) {
	die("FEHLER, zu wenige Daten!");
}

if ($mysql->addEntry($_GET["weight"], $_GET["temp_int"], $_GET["temp_ext"], $_GET["sid_ext"], $_GET["sensetime"], $_GET["chipmillis"], dectohex($_GET["chipid"], 32), $_GET["vcc"], $_GET["ver"], $_GET["mac"])) {
	$device = $mysql->getDevice($_GET["mac"]);
	if($_GET["dbg"] != 1) {
		if(trim($device["ver_upd"]) != "" && $device["ver_upd"] != $device["ver"]) {
			die("Update");
		} else {
			die("Erfolg");
		}
